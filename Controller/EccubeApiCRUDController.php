...
class EccubeApiCRUDController extends AbstractApiController
{
    ...
    public function findAll(Application $app, Request $request, $table = null)
    {
        ...
        $excludes = array();
        if (is_array($AccessToken) && is_object($AccessToken['user'])) {
            // Customer で認証されている場合
            if ($AccessToken['user']->hasCustomer()) {
                switch ($table) {
                    case 'customer':
                        $searchConditions['id'] = $AccessToken['user']->getCustomer()->getId();
                        break;

                    case 'customer_address':
                    case 'order':
                        $searchConditions['Customer'] = $AccessToken['user']->getCustomer();
                        break;
                    case 'order_detail':
                        // Customer のみに絞る
                        break;
        ...
        if ($table == 'order_detail') { // FIXME EC-CUBE本体側に実装すべき
            $qb = $Repository->createQueryBuilder('od')
                ...
                ->setParameter('Customer', $AccessToken['user']->getCustomer());
    ...
    protected function findEntity(Application $app, Request $request, callable $callback = null, array $params_arr = array(), $table = null)
    {
        ...
        if (is_array($AccessToken) && is_object($AccessToken['user'])) {
            ...
            if ($AccessToken['user']->hasCustomer()) {
                ...
                switch ($table) {
                    case 'customer':
                        if ($id != $AccessToken['user']->getCustomer()->getId()) {
                            return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                        }
                        break;
                    case 'customer_address':
                    case 'order':
                        if (is_array($Results) && $Results[$table]['Customer']['id'] != $AccessToken['user']->getCustomer()->getId()) {
                            return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                        }
                        break;
                    case 'order_detail':
                        ...
                        if ($Order->getCustomer()->getId() != $AccessToken['user']->getCustomer()->getId()) {
                            return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                        }
                }
                $excludes= $this->attribureExclusion($table, true);
    ...
    protected function createEntity(Application $app, Request $request, callable $callback, $table = null)
    {
        ...
        // Customer で認証されている場合
        $AccessToken = $this->getAccessToken($app, $request);
        if (is_array($AccessToken) && $AccessToken['user']->hasCustomer()) {
            switch ($table) {
                case 'customer':
                    return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                    break;
                case 'customer_address':
                    if ($Entity->getCustomer()->getId() != $AccessToken['user']->getCustomer()->getId()) {
                        return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                    }
                    break;
                default:
    ...
    public function updateEntity(Application $app, Request $request, callable $callback, array $params_arr = array(), $table = null)
    {
        ...
        if (is_array($AccessToken) && $AccessToken['user']->hasCustomer()) {
            switch ($table) {
                case 'customer':
                    if ($Entity->getId() != $AccessToken['user']->getCustomer()->getId()) {
                        return $this->getWrapperedErrorResponseBy($app, 'Access Forbidden', 403);
                    }
                    $excludes = array(
                        'Country',
